...
class xstandardnumber extends xid {
  ...
  const VALID_OPTIONS      = 'format, callback, fl, hash, token';
  const VALID_FLS          = 'lccn, oclcnum, owi, presentOclcnum, url, *';
  const VALID_NUMBER_TYPES = 'lccn, oclcnum, owi';
   ...
   *
   * @access public
   * @param string $number_type Number type to search by. Valid options are listed in self::VALID_NUMBER_TYPES.
   ...
   * @param string $number_type Type of number being searched. Valid values are listed in self::VALID_NUMBER_TYPES.
  ...
  private function set_options($options = null) {
    if(is_null($options)) {
      ...
      throw new \OCLC\OCLCException('xISSN options must be passed as an array. Valid values include ' $this->constant_to_string(self::VALID_OPTIONS) . '.');
    }
  }
   ...
   *
   * @access private
   * @param array $search Search options. Valid values are listed in self::VALID_OPTIONS.
  ...
  private function validate_number_type($number_type) {
    if(in_array($number_type, $this->constant_to_array(self::VALID_NUMBER_TYPES))) {
      return true;
    } else {
      throw new \OCLC\OCLCException('Invalid number type. Valid values include ' . $this->constant_to_string(self::VALID_NUMBER_TYPES) . '.');
      return false;
    }
  }
  ...
  private function validate_standard_number_fls($fls) {
    $fl = explode(',', $fls);
    foreach($fl as $value) {
      if(!in_array(trim($value), $this->constant_to_array(self::VALID_FLS))) {
        throw new \OCLC\OCLCException('Invalid `fl`. Valid values include ' . $this->constant_to_string(self::VALID_FLS) . '.');
        return false;
      }
    }
    return true;
